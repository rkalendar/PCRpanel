...
public class ExonPromoterExtractor {
    ...
    private final String accession;   // NG_034200
    private final String version;     // NG_034200.2
    private final String recordId;    // version если есть, иначе accession

    public ExonPromoterExtractor(String arg) throws Exception {
        this(arg, 1000);
    }

    public ExonPromoterExtractor(String arg, int promoterLen) throws Exception {
...
// Global sorting of the final list by coordinates (as requested)
    ...
    private static void processFeature(Feature f, int[] seqLenBox, int promoterLen, List<Region> out) throws IOException {
        // 0) infer seqLen from source feature if LOCUS was not parsed
        ...
        // 1) IDs
        String gene = firstNonEmpty(f.getFirst("gene"), f.getFirst("locus_tag"), f.getFirst("gene_synonym"), "NA");
        String transcriptId = firstNonEmpty(f.getFirst("transcript_id"), f.getFirst("product"), f.getFirst("protein_id"), ""
        );

        // 2) Exon models:
        //    - explicit "exon" features
        //    - "mRNA"/"ncRNA" join(...) location segments
        switch (f.key) {
            case "ncRNA" -> {
                LocationParsed lp = parseLocation(f.location.toString());
                if (lp.segments == null || lp.segments.isEmpty()) {
                    return;
                }

                List<int[]> segs = new ArrayList<>(lp.segments);

                // EXONS from joined segments
                addExonsFromSegments(transcriptId, gene, lp.strand, segs, null, out);

                // PROMOTER (computed from TSS)
                if (promoterLen > 0) {
            ...
            case "exon" -> {
                ...
                String exonNumber = firstNonEmpty(f.getFirst("number"), f.getFirst("exon"), null);

                // explicit exon features: usually 1 segment; if multiple, index sequentially
                addExonsFromSegments(transcriptId, gene, lp.strand, lp.segments, exonNumber, out);
                return;
            }
    ...
    private static void addExonsFromSegments(String transcriptId, String gene, char strand,
            List<int[]> segsIn, String singleExonIdx, List<Region> out) {

        List<int[]> segs = new ArrayList<>(segsIn);

        // transcription order (for indexing)
        if (strand == '+') {
        ...
        for (int[] s : segs) {
            String idx;
            if (singleExonIdx != null && !singleExonIdx.isBlank() && segs.size() == 1) {
                idx = singleExonIdx.trim(); // keep /number if provided
            } else {
                idx = String.valueOf(exonIdx);
            }
            out.add(new Region(transcriptId, gene, "EXON", idx, s[0], s[1], strand));
            exonIdx++;
        }
    }
